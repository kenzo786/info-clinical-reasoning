...
3.5. Use `jitl-link-01` to `jitl-link-10` after major sections.
3.6. Use reusable inline JITL shortcodes:
   - `jitl-term-link`
   - `jitl-drug-link`
   - `jitl-guideline-link`
   - `jitl-assessment-link`
4. Keep the map shorter than the reference, but make it consultation-usable for novices.
...
```md
[sc name="map-steps-and-links" id="POST_ID" anchor="101" title="Overview" color="green" step="1" info-modal="" pathway="display-none" youtube="display-none"][/sc]<details class="kb-accordion kb-parent d-green" open> <summary id="s-is.101" role="button">Overview</summary> <ul> <li><strong>Clinical challenge:</strong> ...</li> <li><strong>Guideline frame:</strong> ...</li> <li><strong>Core stance:</strong> ...</li> </ul> </details> [sc name="jitl-link-01" Q="TOPIC"][/sc]
```
...
## Inline JITL Templates

```md
[sc name="jitl-term-link" Q="Pulmonary embolism clinical assessment and management" text="PE"][/sc]
[sc name="jitl-drug-link" drug="Ibuprofen" text="ibuprofen"][/sc]
[sc name="jitl-guideline-link" Q="Headache" text="EXPLORE Headache Guidelines"][/sc]
```
